...
#SBATCH --job-name=organ_proteomics_asthma_clock
#SBATCH --mem-per-cpu=24G
#SBATCH --array=0-10
#SBATCH --output=/cbica/home/wenju/output/organ_proteomics_asthma_clock_%A_%a.out
#SBATCH --error=/cbica/home/wenju/output/organ_proteomics_asthma_clock_%A_%a.err
...
outdir="/cbica/home/wenju/Reproducibile_paper/WholeBodyClock/${organ}_proteomics_asthma_clock"
mkdir -p "${outdir}"
mkdir -p /cbica/home/wenju/output

# Redirect main job log dynamically into each organ-specific output folder
exec > "${outdir}/${organ}_proteomics_asthma_clock_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}.out" \
     2> "${outdir}/${organ}_proteomics_asthma_clock_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err"

echo "============================================================"
echo "Starting organ proteomics asthma clock"
echo "Organ: ${organ}"
...
echo "============================================================"
echo "Finished organ proteomics asthma clock"
echo "Organ: ${organ}"
