# ============================================================
# Asthma L'EPOCH model performance visualization
...
#
# Example:
#   brain_mri_asthma_clock/
#     brain_mri_asthma_clock_predictions.tsv
#     brain_mri_asthma_clock_performance.json
#     brain_mri_asthma_clock_model_comparison.tsv
#     brain_mri_asthma_clock_incremental_value_delta_cindex.tsv
# ============================================================
...
combined_outdir <- file.path(base_dir, "all_asthma_lepoch_model_performance")
...
make_manifest <- function() {
  ...
  bind_rows(
    tibble(
      ...
      folder = paste0(stringr::str_to_lower(mri_organs), "_mri_asthma_clock"),
      prefix = paste0(stringr::str_to_lower(mri_organs), "_mri_asthma_clock")
    ),
    tibble(
      modality = "Proteomics",
      ...
      folder = paste0(proteomics_organs, "_proteomics_asthma_clock"),
      prefix = paste0(stringr::str_to_lower(proteomics_organs), "_proteomics_asthma_clock")
    ),
    tibble(
      ...
      folder = paste0(metabolomics_organs, "_metabolomics_asthma_clock"),
      prefix = paste0(stringr::str_to_lower(metabolomics_organs), "_metabolomics_asthma_clock")
    )
  ) %>%
    mutate(
...
message("Expected Asthma L'EPOCH clocks: ", nrow(clock_manifest))
...
calc_cindex <- function(df) {
  ...
  # Larger L'EPOCH risk score means higher asthma risk, so use -risk_score.
...
# ============================================================
# 4. Asthma L'EPOCH column detection
# ============================================================

detect_asthma_columns <- function(pred, meta) {
  feature_token <- paste0(meta$organ_key, "_", meta$modality_key)

  expected_risk_score_col <- paste0(feature_token, "_asthma_risk_score")
  expected_accel_z_col <- paste0(feature_token, "_asthma_clock_acceleration_z")
  expected_accel_year_col <- paste0(feature_token, "_asthma_clock_acceleration_years")
  expected_clock_age_col <- paste0(feature_token, "_asthma_clock_age_years")
  ...
  risk_score_col <- first_existing_col(pred, c(
    ...
    paste0(feature_token, "_asthma_lepoch_risk_score")
  ))
  ...
  if (is.na(risk_score_col)) {
    risk_score_col <- first_regex_col(
      pred,
      patterns = c(
        paste0("^", feature_token, ".*asthma.*risk.*score$"),
  ...
  accel_z_col <- first_existing_col(pred, c(
    ...
    paste0(feature_token, "_asthma_acceleration_z"),
  ...
  accel_year_col <- first_existing_col(pred, c(
    ...
    paste0(feature_token, "_asthma_acceleration_years"),
  ...
  clock_age_col <- first_existing_col(pred, c(
    ...
    paste0(feature_token, "_asthma_clock_age_years"),
...
plot_one_clock <- function(meta_row, horizon_years = 5, n_boot = 500, skip_missing = TRUE) {
  ...
  if (!file.exists(meta$clock_dir) || !file.exists(meta$prediction_file) || !file.exists(meta$performance_file)) {
    msg <- paste0(
      "Missing input for ", meta$clock_label, " Asthma L'EPOCH:\n",
  ...
  message("\n============================================================")
  message("Processing ", meta$clock_label, " Asthma L'EPOCH")
  ...
  detected <- detect_asthma_columns(pred, meta)
  ...
  if (is.na(risk_score_col) || !risk_score_col %in% colnames(pred)) {
    stop(
      "Could not identify Asthma L'EPOCH risk-score column for ",
      meta$clock_label,
  ...
  message("Asthma events total: ", sum(pred$event, na.rm = TRUE))
  ...
  # Preserve original per-clock folder, but use asthma-specific figure suffix.
  ...
  if (sum(test_for_km$event, na.rm = TRUE) >= 2) {
    ...
    km_df <- tibble()
    warning(
      "Too few test-set asthma events for Kaplan-Meier by quartile for ",
      meta$clock_label,
      "."
    )
  }
  ...
  p_density <- pred %>%
    ...
    labs(
      ...
      x = glue("{meta$clock_label} Asthma L'EPOCH risk score"),
      y = "Density"
    ) +
    theme_clock()
  ...
  if (nrow(km_df) > 0) {
    p_km <- km_df %>%
      ...
      labs(
        title = "D. Test-set asthma separation",
        subtitle = "Kaplan-Meier cumulative asthma incidence by training-defined risk quartile",
        x = glue("Years after {time_origin_label}"),
        y = "Cumulative asthma incidence"
      ) +
      theme_clock()
  } else {
    p_km <- ggplot() +
      annotate(
        ...
        x = 0,
        y = 0,
        label = "Kaplan-Meier panel unavailable: too few test-set asthma events.",
        size = 4.5,
        fontface = "bold"
      ) +
      theme_void() +
      labs(title = "D. Test-set asthma separation")
  }
  ...
  p_quartile <- quartile_risk_tbl %>%
    ...
    scale_y_continuous(labels = percent_format(accuracy = 0.1)) +
    labs(
      title = glue("E. Observed {local_horizon_years}-year asthma risk by risk quartile"),
      subtitle = "Monotonic validation/test patterns support generalization",
      x = NULL,
      y = glue("Observed {local_horizon_years}-year asthma risk")
    ) +
    theme_clock() +
  ...
  if (nrow(cal_tbl) > 0) {
    ...
    p_cal <- cal_tbl %>%
      ...
      labs(
        ...
        subtitle = "Observed Kaplan-Meier asthma risk versus predicted Cox risk by decile",
        x = glue("Mean predicted {local_horizon_years}-year asthma risk"),
        y = glue("Observed {local_horizon_years}-year asthma risk")
      ) +
      theme_clock()
  } else {
  ...
  subtitle_text <- glue(
    "N = {nrow(pred)}, asthma events = {sum(pred$event)}, ",
  ...
  final_fig <-
    ...
    (p_quartile | p_cal) +
    plot_annotation(
      title = glue("{meta$clock_label} Asthma L'EPOCH performance"),
      subtitle = subtitle_text,
      caption = paste0(
        "Risk quartiles are defined from the training set and applied to validation/test. ",
        "Higher L'EPOCH risk score indicates greater asthma-event proximity."
      ),
      theme = theme(
...
# ============================================================
# 6. Loop over all Asthma L'EPOCH clocks
# ============================================================
...
if (length(results) == 0) {
  stop("No Asthma L'EPOCH clocks were successfully processed.")
}
...
out_combined_summary <- file.path(
  combined_outdir,
  "all_asthma_lepoch_summary.tsv"
)

out_combined_overfit <- file.path(
  combined_outdir,
  "all_asthma_lepoch_overfitting_summary.tsv"
)

out_combined_quartile <- file.path(
  combined_outdir,
  "all_asthma_lepoch_risk_quartile_summary.tsv"
)

out_combined_calibration <- file.path(
  combined_outdir,
  "all_asthma_lepoch_calibration_summary.tsv"
)

out_combined_cindex <- file.path(
  combined_outdir,
  "all_asthma_lepoch_cindex_summary.tsv"
)

out_combined_incremental <- file.path(
  combined_outdir,
  "all_asthma_lepoch_incremental_value_summary.tsv"
)

message("\n============================================================")
message("Writing combined Asthma L'EPOCH outputs")
message("============================================================")
...
base_quality <- combined_summary_plot_tbl %>%
  ...
  labs(
    title = "D. Practical Asthma L'EPOCH model-quality map",
...
combined_fig <-
  ...
  plot_annotation(
    title = "Performance summary of MRI, proteomics, and metabolomics Asthma L'EPOCH models",
    subtitle = glue(
      "Processed {nrow(combined_summary)} of {nrow(clock_manifest)} expected Asthma L'EPOCH models. ",
...
out_combined_pdf <- file.path(
  combined_outdir,
  "all_asthma_lepoch_combined_model_performance.pdf"
)

out_combined_png <- file.path(
  combined_outdir,
  "all_asthma_lepoch_combined_model_performance.png"
)
...
message("\n===== Combined Asthma L'EPOCH summary =====")
...
message("  - Per-clock PDF/PNG figures and summary TSVs are saved in each Asthma L'EPOCH folder.")
