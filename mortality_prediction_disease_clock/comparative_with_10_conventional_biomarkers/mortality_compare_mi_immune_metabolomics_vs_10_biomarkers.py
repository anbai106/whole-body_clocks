...
Landmark definition follows the existing disease-clock mortality analysis:
    metabolomics EPOCH -> UKB Field 53 instance 0_0
...
# =============================================================================
# 2. Resolve the target MI immune-metabolomics EPOCH
# =============================================================================
...
def resolve_target_clock(
    ...
    if score_col_override:
        ...
        return {
            "disease": disease_key,
            "folder": "",
            "clock_label": "{} {} {} EPOCH".format(disease_key, organ_key, modality_key),
    ...
    disease_aliases = {disease_norm}
    if disease_norm in {"mi", "myocardial infarction", "myocardial_infarction", "myocardial-infarction"}:
        disease_aliases.update({
            "mi",
    ...
    disease_series = meta["disease"].astype(str).str.strip().str.lower()
    candidates = meta[disease_series.isin(disease_aliases)].copy()

    if candidates.empty:
        raise ValueError(
            "No metadata rows found for disease key '{}' (aliases: {}).".format(
                disease_key, sorted(disease_aliases)
            )
        )

    # Search the combined metadata text for the requested organ and modality.
    organ_terms = [str(organ_key).strip().lower()]
    if str(organ_key).strip().lower() == "immune":
        organ_terms.extend(["immune", "immunity"])
    ...
    if candidates.empty:
        raise ValueError(
            "Could not resolve the MI immune-metabolomics clock from metadata. "
    ...
    if len(resolved_rows) == 0:
        raise ValueError(
            "Candidate MI immune-metabolomics metadata row(s) were found, "
            "but no score column could be resolved in the wide score TSV."
        )

    if len(resolved_rows) > 1:
        msg = [
            "Multiple MI immune-metabolomics score columns were resolved:",
        ]
        for r in resolved_rows:
            msg.append(
...
def run(args):
    ...
    if modality.lower() != "metabolomics":
        raise ValueError(
            "Resolved target is not metabolomics: modality={}".format(
                modality
            )
        )
    ...
    prefix = "mi_immune_metabolomics_EPOCH_vs_10_biomarkers_mortality"
    ...
    info(
        "Non-missing MI immune-metabolomics EPOCH: {:,}".format(
    ...
    # -------------------------------------------------------------------------
    # Mortality follow-up: same landmark rule as existing metabolomics analysis.
    # -------------------------------------------------------------------------
    ...
    predictor_specs = [
        {
            "column": "epoch_raw",
            "display": "MI immune-metabolomics EPOCH",
            "type": "EPOCH",
            "source": score_col,
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Compare MI immune-metabolomics EPOCH with 10 conventional "
    ...
    parser.add_argument("--disease-key", default="mi")
    parser.add_argument("--organ-key", default="immune")
    parser.add_argument("--modality-key", default="metabolomics")
    parser.add_argument(
        "--score-col",
        default=None,
        help=(
            "Optional exact MI immune-metabolomics EPOCH score column. "
