#!/usr/bin/env python3
"""
General organ MRI dementia clock using elastic-net Cox survival modeling.
...
Output naming is controlled by --organ, e.g. --organ heart creates:
  heart_mri_dementia_clock_predictions.tsv
  heart_mri_dementia_clock_performance.json
  etc.
"""
...
def parse_args():
    ...
    p.add_argument("--dementia-xlsx", default="/cbica/home/wenju/Dataset/UKBB_UMelbourne/Death_related_var_from_Ye.xlsx")
...
def output_prefix(organ):
    return f"{organ}_mri_dementia_clock"
...
def load_dementia_data(dementia_xlsx, id_match_csv):
    ...
    # Keep it if available so dementia-free participants can be censored at death.
    if "40000-0.0" in d.columns:
        keep.append("40000-0.0")
    else:
        warnings.warn(
            "Death date field 40000-0.0 was not found. "
            "Dementia-free participants will be censored only at the administrative censor date."
        )
...
def construct_survival_dataset(df):
    ...
    Event:
        Incident dementia after imaging and before censoring.

    Exclusions:
        - Missing imaging date
        - Dementia before or on imaging date
    ...
    # Incident dementia must occur after imaging and on/before censor date.
    df["event"] = (
        df["dementia_date"].notna()
        & (df["dementia_date"] > df["imaging_date"])
...
def add_clock_age_and_acceleration(pred_df, organ, covariate_cols):
    df = pred_df.copy()
    risk_col = f"{organ}_mri_dementia_risk_score"
    z_col = f"{organ}_mri_dementia_clock_acceleration_z"
    yrs_col = f"{organ}_mri_dementia_clock_acceleration_years"
    age_col = f"{organ}_mri_dementia_clock_age_years"
...
def main():
    ...
    print(f"Building {organ} MRI dementia clock")
    ...
    print("Merging data...")
    df = organ_df.merge(dementia, on="participant_id", how="inner")
    if cov is not None:
    ...
    if df["event"].sum() < 20:
        warnings.warn("Very few dementia events. The fitted clock may be unstable.")

    dataset_cols = ["participant_id", "baseline_date", "imaging_date", "dementia_date", "admin_censor_date", "end_date", "time_days", "time_years", "event", "age_at_imaging", "sex"]
    ...
    def make_pred_frame(part, split, risk):
        base = ["participant_id", "imaging_date", "dementia_date", "admin_censor_date", "end_date", "time_years", "event", "age_at_imaging", "sex"]
    ...
    print("Adding approximate dementia-clock age and acceleration...")
