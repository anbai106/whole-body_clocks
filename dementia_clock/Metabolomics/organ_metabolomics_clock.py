#!/usr/bin/env python3
"""
General organ metabolomics dementia clock using elastic-net Cox survival modeling.
...
Output naming is controlled by --organ, e.g. --organ heart creates:
  heart_metabolomics_dementia_clock_predictions.tsv
  heart_metabolomics_dementia_clock_performance.json
  etc.
"""
...
def output_prefix(organ):
    return f"{organ}_metabolomics_dementia_clock"


def load_dementia_data(dementia_xlsx, id_match_csv):
    """
    Load UKB assessment dates, dementia date, and death date.

    Metabolomics is treated as baseline omics, so time zero is the baseline
    assessment date from UMelbourne field 53-0.0. Dementia event date is
    UKB field 42018-0.0. Death date, if available, is used as a competing
    censoring time for dementia-free participants.
    """
    ...
    required = ["participant_id", "53-0.0", "42018-0.0"]
...
def construct_survival_dataset(df):
    """
    Construct prospective dementia survival outcome using baseline sample date as time zero.

    Event:
        Incident dementia after baseline sample date and before censoring.

    Exclusions:
        - Missing baseline/sample date
        - Dementia before or on baseline sample date
    ...
    df["dementia_before_or_on_sample"] = (
        df["dementia_date"].notna()
        & df["sample_date"].notna()
        & (df["dementia_date"] <= df["sample_date"])
    )

    n_pre = int(df["dementia_before_or_on_sample"].sum())
    if n_pre:
        warnings.warn(f"Excluding {n_pre} participants with dementia before/on baseline sample date.")

    df = df.loc[df["sample_date"].notna()].copy()
    df = df.loc[~df["dementia_before_or_on_sample"]].copy()
    ...
    # Incident dementia must occur after sample date and on/before censor date.
    df["event"] = (
        df["dementia_date"].notna()
        & (df["dementia_date"] > df["sample_date"])
...
def add_clock_age_and_acceleration(pred_df, organ, covariate_cols):
    df = pred_df.copy()
    risk_col = f"{organ}_metabolomics_dementia_risk_score"
    z_col = f"{organ}_metabolomics_dementia_clock_acceleration_z"
    yrs_col = f"{organ}_metabolomics_dementia_clock_acceleration_years"
    age_col = f"{organ}_metabolomics_dementia_clock_age_years"
...
def main():
    ...
    print(f"Building {organ} metabolomics dementia clock")
    ...
    print("Final prospective baseline-omics dementia dataset:")
    print(f"  N = {df.shape[0]}")
    print(f"  Incident dementia events after baseline = {int(df['event'].sum())}")
    ...
    dataset_cols = ["participant_id", "baseline_date", "sample_date", "dementia_date", "death_date", "admin_censor_date", "censor_date", "end_date", "time_days", "time_years", "event", "age_at_baseline", "age_at_imaging", "sex"]
    ...
    def make_pred_frame(part, split, risk):
        base = ["participant_id", "sample_date", "dementia_date", "death_date", "admin_censor_date", "censor_date", "end_date", "time_years", "event", "age_at_baseline", "age_at_imaging", "sex"]
