...
organ_list = c("Metabolic", "Immune")
output_dir = '/Users/hao/cubic-home/Reproducibile_paper/UKBB_metabolomics/fastGWA_MetBAG/data/'

for (organ in organ_list) {
  ## female
  output_dir_final = paste(output_dir, organ, 'female', sep='/')
  data_src = paste(output_dir, organ, 'female', 'BAG_pheno.txt', sep='/')
  cov_src = paste(output_dir, organ, 'female', 'BAG_cov.txt', sep='/')
  lm_imaging_data(data_src, cov_src, output_dir_final)

  ### male
  print(paste0("Organ: ", organ))
  output_dir_final = paste(output_dir, organ,  'male', sep='/')
  data_src = paste(output_dir, organ,  'male', 'BAG_pheno.txt', sep='/')
  cov_src = paste(output_dir, organ,  'male', 'BAG_cov.txt', sep='/')
  lm_imaging_data(data_src, cov_src, output_dir_final)
}
