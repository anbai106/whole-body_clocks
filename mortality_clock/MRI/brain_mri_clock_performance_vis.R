...
clock_dir <- "/Users/hao/cubic-home/Reproducibile_paper/WholeBodyClock/brain_mri_mortality_clock"

prediction_file <- file.path(clock_dir, "brain_mri_mortality_clock_predictions.tsv")
performance_file <- file.path(clock_dir, "brain_mri_mortality_clock_performance.json")
...
# Output files
out_pdf <- file.path(clock_dir, "brain_mri_mortality_clock_model_performance.pdf")
out_png <- file.path(clock_dir, "brain_mri_mortality_clock_model_performance.png")
out_overfit <- file.path(clock_dir, "brain_mri_mortality_clock_overfitting_summary.tsv")
out_quartile <- file.path(clock_dir, "brain_mri_mortality_clock_risk_quartile_summary.tsv")
...
risk_score_col <- "brain_mri_mortality_risk_score"
accel_z_col <- "brain_mri_mortality_clock_acceleration_z"
accel_year_col <- "brain_mri_mortality_clock_acceleration_years"
clock_age_col <- "brain_mri_mortality_clock_age_years"
...
if (!is.null(risk_col)) {
  ...
  if (nrow(cal_input) > 0) {
    ...
    out_cal <- file.path(
      clock_dir,
      paste0("brain_mri_mortality_clock_calibration_", horizon_years, "y.tsv")
    )
