#!/usr/bin/env python3
"""
General organ MRI mortality clock using elastic-net Cox survival modeling.
...
Output naming is controlled by --organ, e.g. --organ heart creates:
  heart_mri_mortality_clock_predictions.tsv
  heart_mri_mortality_clock_performance.json
  etc.
"""
...
def load_death_data(death_xlsx, id_match_csv):
    ...
    need = ["participant_id", "53-0.0", "53-2.0", "40000-0.0"]
...
def construct_survival_dataset(df):
    df = df.copy()
    df["death_before_or_on_imaging"] = df["death_date"].notna() & df["imaging_date"].notna() & (df["death_date"] <= df["imaging_date"])
    n_pre = int(df["death_before_or_on_imaging"].sum())
    if n_pre:
        warnings.warn(f"Excluding {n_pre} participants with death before/on imaging date.")
    ...
    df["event"] = df["death_date"].notna() & (df["death_date"] > df["imaging_date"]) & (df["death_date"] <= df["admin_censor_date"])
...
def main():
    ...
    print(f"Building {organ} MRI mortality clock")
    ...
    dataset_cols = ["participant_id", "baseline_date", "imaging_date", "death_date", "admin_censor_date", "end_date", "time_days", "time_years", "event", "age_at_imaging", "sex"]
    ...
    def make_pred_frame(part, split, risk):
        base = ["participant_id", "imaging_date", "death_date", "admin_censor_date", "end_date", "time_years", "event", "age_at_imaging", "sex"]
