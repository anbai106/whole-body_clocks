...
"""
Build a heart MRI mortality clock in UK Biobank using elastic-net Cox survival modeling.

Inputs:
  1. UKB death/assessment-date Excel file from UMelbourne
  2. UKB UMelbourne-to-Penn participant ID matching file
  3. Heart MRI feature TSV file(s). The --heart-tsv argument may be either:
       - one already-concatenated TSV file, or
       - a comma-separated list of TSV files that will be concatenated internally
...
Heart-specific covariates used here:
  - age at imaging visit: age_when_attended_assessment_centre_f21003_2_0
  - sex: sex_f31_0_0
  - BMI at imaging visit: body_mass_index_bmi_f23104_2_0
  - mean diastolic BP at imaging visit: fields 4079-2.0 and 4079-2.1
  - mean systolic BP at imaging visit: fields 4080-2.0 and 4080-2.1
  - assessment center at imaging visit: uk_biobank_assessment_centre_f54_2_0

Example:
  python heart_mri_clock.py \
    ...
    --heart-tsv /path/to/heart_train_val_test.tsv,/path/to/heart_independent_test.tsv,/path/to/heart_patient_mlni.tsv \
...
HEART_DEFAULT_TSVS = ",".join(
    [
        "/cbica/home/wenju/Reproducibile_paper/AbdoImaging/Data_split_by_Huizi/MLNI_dataset_splits/heart_train_val_test.tsv",
        "/cbica/home/wenju/Reproducibile_paper/AbdoImaging/Data_split_by_Huizi/MLNI_dataset_splits/heart_independent_test.tsv",
        "/cbica/home/wenju/Reproducibile_paper/AbdoImaging/Data_split_by_Huizi/raw_data_MLNI/heart_patient_mlni.tsv",
    ]
)
...
def parse_args():
    ...
    parser.add_argument(
        "--heart-tsv",
        default=HEART_DEFAULT_TSVS,
        help=(
            "Heart MRI feature TSV. This can be one concatenated TSV or a comma-separated "
            "list of TSV files; comma-separated files are concatenated internally."
        ),
    )
    parser.add_argument(
        "--covariate-csv",
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Administrative censor date for death registry follow-up, e.g. 2022-11-30. "
    ...
    parser.add_argument(
        ...
        default="ses-2",
        help=(
            "Heart imaging session_id to use from the heart TSV. Default is ses-2. "
            "Use 'none' to disable session filtering."
        ),
    )
    parser.add_argument(
        "--test-size",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.20,
        help="Drop heart imaging/covariate features with missingness above this threshold in the training set.",
    )
    parser.add_argument(
        "--l1-ratios",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "heart MRI model improves test-set C-index over the covariate baseline."
        ),
    )
...
def expand_heart_tsv_paths(heart_tsv_arg):
    """
    Allow --heart-tsv to be one path, a comma-separated list of paths, or glob(s).
    """
    if heart_tsv_arg is None or str(heart_tsv_arg).strip() == "":
        raise ValueError("--heart-tsv cannot be empty.")

    pieces = [p.strip() for p in str(heart_tsv_arg).split(",") if p.strip()]
    paths = []
    for piece in pieces:
        expanded = sorted(glob.glob(piece)) if any(ch in piece for ch in ["*", "?", "["]) else [piece]
        if len(expanded) == 0:
            raise FileNotFoundError(f"No files matched heart TSV pattern: {piece}")
...
def load_heart_data(heart_tsv, imaging_session_id="ses-2"):
    paths = expand_heart_tsv_paths(heart_tsv)

    frames = []
    for source_order, path in enumerate(paths):
        if not os.path.exists(path):
            raise FileNotFoundError(f"Heart TSV file not found: {path}")
        df_part = pd.read_csv(path, sep="\t")
        if "participant_id" not in df_part.columns:
            raise ValueError(f"Heart TSV must contain participant_id: {path}")
        df_part["heart_source_file"] = Path(path).name
        df_part["heart_source_order"] = source_order
        df_part["heart_source_row"] = np.arange(df_part.shape[0])
        frames.append(df_part)
        print(f"Loaded heart TSV: {path}, rows={df_part.shape[0]}, cols={df_part.shape[1]}")

    df_heart = pd.concat(frames, axis=0, ignore_index=True, sort=False)
    print(f"Concatenated heart feature files: rows={df_heart.shape[0]}, cols={df_heart.shape[1]}")

    if imaging_session_id is not None and str(imaging_session_id).lower() not in ["none", "", "null"]:
        if "session_id" in df_heart.columns:
            before = df_heart.shape[0]
            df_heart = df_heart.loc[df_heart["session_id"].astype(str) == str(imaging_session_id)].copy()
            after = df_heart.shape[0]
            print(f"Filtered heart file to session_id={imaging_session_id}: {before} -> {after} rows")
        else:
            warnings.warn("--imaging-session-id was provided, but heart TSV does not contain session_id.")

    # Keep one imaging row per participant. If a participant appears in multiple source files,
    # we keep the first occurrence in the user-specified input order:
    # train/val/test -> independent test -> patient, unless --heart-tsv is changed.
    sort_cols = ["participant_id", "heart_source_order", "heart_source_row"]
    df_heart = df_heart.sort_values(sort_cols, kind="mergesort")

    duplicated = df_heart["participant_id"].duplicated().sum()
    if duplicated > 0:
        warnings.warn(
            f"Found {duplicated} duplicated participant_id rows in concatenated heart files. "
            "Keeping the first row per participant."
        )
        df_heart = df_heart.drop_duplicates("participant_id", keep="first")
...
def infer_feature_columns(df):
    """
    Infer UKB heart MRI features. The expected heart features are UKB field IDs
    f24100-f24181 at imaging instance 2, with names such as
    lv_end_diastolic_volume_f24100_2_0.
    """
    heart_feature_cols = [
        c for c in df.columns
        if re.search(r"_f24\d+_2_0$", str(c)) is not None
    ]

    excluded = {"participant_id", "session_id", "diagnosis", "heart_source_file"}
    heart_feature_cols = [c for c in heart_feature_cols if c not in excluded]

    if len(heart_feature_cols) == 0:
        raise ValueError(
            "No heart MRI feature columns found. Expected names ending with '_f24..._2_0', "
            "such as lv_end_diastolic_volume_f24100_2_0."
        )

    return heart_feature_cols
...
def mean_existing_numeric_columns(df, cols, out_col):
    ...
    df[out_col] = df[cols_present].mean(axis=1, skipna=True)
    return df
...
def add_basic_covariates(df):
    ...
    # Heart imaging is taken at UKB imaging visit / instance 2.
    age_col = "age_when_attended_assessment_centre_f21003_2_0"
    if age_col in df.columns:
        df["age_at_imaging"] = pd.to_numeric(df[age_col], errors="coerce")
    elif "age_at_imaging" in df.columns:
        df["age_at_imaging"] = pd.to_numeric(df["age_at_imaging"], errors="coerce")
    elif "diagnosis" in df.columns:
        warnings.warn(
            "Could not find age_when_attended_assessment_centre_f21003_2_0. "
            "Using numeric 'diagnosis' column as age_at_imaging fallback because the heart file appears to use diagnosis as age. "
            "Please verify this is correct."
        )
        df["age_at_imaging"] = pd.to_numeric(df["diagnosis"], errors="coerce")
    else:
        raise ValueError(
            "Could not infer age_at_imaging. Expected age_when_attended_assessment_centre_f21003_2_0 "
            "from the covariate file."
        )

    sex_col = "sex_f31_0_0"
    if sex_col in df.columns:
        df["sex"] = df[sex_col].astype(str).str.strip()
    elif "Sex" in df.columns:
        df["sex"] = df["Sex"].astype(str).str.strip()
    elif "sex" in df.columns:
        df["sex"] = df["sex"].astype(str).str.strip()
    else:
        raise ValueError("Could not infer sex. Expected sex_f31_0_0 from the covariate file.")
    ...
    if "body_mass_index_bmi_f23104_2_0" in df.columns:
        df["bmi_at_imaging"] = pd.to_numeric(df["body_mass_index_bmi_f23104_2_0"], errors="coerce")
    ...
    if "uk_biobank_assessment_centre_f54_2_0" in df.columns:
        df["uk_biobank_assessment_centre_f54_2_0"] = df[
...
def build_design_matrix(df, heart_feature_cols):
    """
    Define covariates and feature columns.

    The model includes age, sex, heart-specific covariates, assessment center,
    and heart MRI features.

    We report a clock-acceleration score by residualizing the final risk score
    against all retained non-heart covariates in the training set.
    """
    df = df.copy()
    ...
    # Convert heart features to numeric.
    for c in heart_feature_cols:
        df[c] = pd.to_numeric(df[c], errors="coerce")

    numeric_cols = numeric_covariates + heart_feature_cols
    categorical_cols = categorical_covariates

    return df, numeric_cols, categorical_cols, numeric_covariates, heart_feature_cols
...
def is_heart_feature_name(feature_names):
    """
    Identify transformed heart MRI features from ColumnTransformer feature names.
    """
    ...
    prefixes = (
        ...
        "num__la_",
        "num__ra_",
        "num__ascending_aorta_",
        "num__descending_aorta_",
    )
    is_heart = np.zeros(len(feature_names), dtype=bool)
    for prefix in prefixes:
        is_heart |= np.char.startswith(feature_names, prefix)
    # Extra safety: all expected heart variables contain a UKB f24xxx field ID.
    is_heart |= np.array([re.search(r"_f24\d+_2_0$", f) is not None for f in feature_names])
    return is_heart
...
def fit_and_select_coxnet(
    ...
    Heart MRI features are assigned penalty_factor=1.
    """
    del random_state

    penalty_factor = np.ones(len(feature_names), dtype=float)
    is_heart_feature = is_heart_feature_name(feature_names)
    penalty_factor[~is_heart_feature] = 0.0
    ...
    for l1_ratio in l1_ratios:
        ...
        try:
            ...
            warnings.warn(
                ...
                "Covariates will be penalized together with heart imaging features."
            )
            model = CoxnetSurvivalAnalysis(
                l1_ratio=l1_ratio,
                n_alphas=n_alphas,
...
def get_incremental_model_feature_indices(feature_names):
    ...
    M0: age + sex
    M1: all retained non-heart covariates, including age, sex, BMI,
        blood pressure, and assessment center
    M2: heart MRI features only
    M3: full model = non-heart covariates + heart MRI features
    """
    feature_names = np.asarray(feature_names).astype(str)

    is_heart = is_heart_feature_name(feature_names)
    ...
    idx = {
        ...
        "M1_covariate_baseline": np.where(~is_heart)[0],
        "M2_heart_mri_only": np.where(is_heart)[0],
        "M3_full_covariates_plus_heart_mri": np.arange(len(feature_names)),
    }
...
def paired_bootstrap_delta_cindex(y, risk_full, risk_baseline, n_boot=1000, random_state=2026):
    ...
    if boot_deltas.size == 0:
        return {
            "comparison": "M3_full_covariates_plus_heart_mri_vs_M1_covariate_baseline",
    ...
    if observed_delta > 0 and ci_lower > 0:
        interp = "Heart MRI improves test-set C-index beyond the covariate baseline."
    elif observed_delta > 0:
        interp = "Heart MRI has positive delta C-index, but the bootstrap CI includes or approaches zero."
    else:
        interp = "No evidence that heart MRI improves test-set C-index beyond the covariate baseline."
...
def run_incremental_value_analysis(
    ...
    """
    Fit baseline comparison models and test whether adding heart MRI features
    ...
    for model_name in ["M0_age_sex", "M1_covariate_baseline", "M2_heart_mri_only"]:
    ...
    fitting_methods["M3_full_covariates_plus_heart_mri"] = "Selected elastic-net Cox model from main pipeline"
    for split, (X_split, _) in split_data.items():
        risk_predictions[split]["M3_full_covariates_plus_heart_mri"] = predict_risk_score(final_model, X_split)

    rows = []
    model_labels = {
        "M0_age_sex": "Age + sex",
        "M1_covariate_baseline": "Covariate baseline",
        "M2_heart_mri_only": "Heart MRI only",
        "M3_full_covariates_plus_heart_mri": "Covariates + heart MRI",
    }
    ...
    for model_name in [
        ...
        "M2_heart_mri_only",
        "M3_full_covariates_plus_heart_mri",
    ]:
            ...
            rows.append(
                {
                    ...
                    "split": split,
                    "n_features": int(len(feature_indices[model_name])),
    ...
    delta_stats = paired_bootstrap_delta_cindex(
        y=y_test,
        risk_full=risk_predictions["test"]["M3_full_covariates_plus_heart_mri"],
...
def add_clock_age_and_acceleration(pred_df, covariate_cols=None, train_mask_col="split"):
    ...
    This is a post-hoc interpretability transform:
      1. Fit risk_score ~ retained non-heart covariates on the training set.
    ...
    # Keep only covariates that are present. Do not include heart MRI features here.
    ...
    if train_df.shape[0] < 10:
        ...
        df["heart_mri_mortality_clock_acceleration_years"] = np.nan
        df["heart_mri_mortality_clock_age_years"] = np.nan
        df["heart_mri_mortality_clock_acceleration_z"] = np.nan
        return df, None
    ...
    lr = LinearRegression()
    lr.fit(X_train, train_df["heart_mri_mortality_risk_score"].values)

    expected = lr.predict(X_all)
    residual_raw = df["heart_mri_mortality_risk_score"].values - expected
    ...
    df["heart_mri_mortality_clock_acceleration_z"] = (
    ...
    if np.isfinite(beta_age) and abs(beta_age) > 1e-8:
        df["heart_mri_mortality_clock_acceleration_years"] = residual / beta_age
        df["heart_mri_mortality_clock_age_years"] = (
            df["age_at_imaging"] + df["heart_mri_mortality_clock_acceleration_years"]
        )
    else:
        warnings.warn(
    ...
    transform_info = {
        ...
        "note": (
            ...
            "for all retained non-heart covariates listed in residualization_covariates. "
...
def main():
    ...
    print("Loading heart MRI data...")
    df_heart = load_heart_data(args.heart_tsv, args.imaging_session_id)

    print("Loading optional covariates...")
    df_cov = load_covariates(args.covariate_csv)

    print("Merging data...")
    df = df_heart.merge(df_death, on="participant_id", how="inner")
    ...
    print("Adding heart-specific covariates...")
    df = add_basic_covariates(df)

    heart_feature_cols = infer_feature_columns(df)
    print(f"Found {len(heart_feature_cols)} heart MRI features.")

    df, numeric_cols, categorical_cols, numeric_covariates, heart_feature_cols = build_design_matrix(
        df, heart_feature_cols
    )
    ...
    if "heart_source_file" in df.columns:
        keep_dataset_cols.append("heart_source_file")

    df[keep_dataset_cols].to_csv(
        outdir / "heart_mri_mortality_survival_dataset.tsv",
        sep="\t",
        index=False,
    )
    ...
    residualization_covariates = [
        ...
        if c not in heart_feature_cols
    ]
    print("Residualizing clock acceleration on retained non-heart covariates:")
    ...
    print("Running incremental-value analysis: does heart MRI add value beyond covariates?")
    ...
    model_comparison_df.to_csv(
        outdir / "heart_mri_mortality_clock_model_comparison.tsv",
        sep="\t",
        index=False,
    )
    delta_cindex_df.to_csv(
        outdir / "heart_mri_mortality_clock_incremental_value_delta_cindex.tsv",
        sep="\t",
        index=False,
    )
    ...
    def make_pred_frame(df_part, split_name, risk):
        ...
        if "heart_source_file" in df_part.columns:
            base_cols.append("heart_source_file")
        ...
        out["heart_mri_mortality_risk_score"] = risk
        return out
    ...
    comparison_risk_cols = {
        ...
        "M2_heart_mri_only": "risk_score_M2_heart_mri_only",
        "M3_full_covariates_plus_heart_mri": "risk_score_M3_full_covariates_plus_heart_mri",
    }
    ...
    pred_all.to_csv(
        outdir / "heart_mri_mortality_clock_predictions.tsv",
        sep="\t",
        index=False,
    )
    pred_test.to_csv(
        outdir / "heart_mri_mortality_clock_test_predictions.tsv",
        sep="\t",
        index=False,
    )
    ...
    is_heart_arr = is_heart_feature_name(feature_names)
    coef_df = pd.DataFrame(
        {
            ...
            "penalty_factor": penalty_factor,
            "is_nonzero": coef != 0,
            "is_heart_mri_feature": is_heart_arr,
        }
    ).sort_values("abs_coefficient", ascending=False)

    coef_df.to_csv(
        outdir / "heart_mri_mortality_clock_coefficients.tsv",
        sep="\t",
        index=False,
    )
    ...
    nonzero_coef_df.to_csv(
        outdir / "heart_mri_mortality_clock_nonzero_coefficients.tsv",
        sep="\t",
        index=False,
    )
    ...
    model_bundle = {
        ...
        "heart_feature_cols": heart_feature_cols,
        ...
        "heart_tsv_input": args.heart_tsv,
        "admin_censor_date": str(admin_censor_date.date()),
    }

    joblib.dump(model_bundle, outdir / "heart_mri_mortality_clock_model.joblib")
    ...
    performance = {
        ...
        "cindex_test_M3_full_covariates_plus_heart_mri": float(
            model_comparison_df.loc[
                (model_comparison_df["model"] == "M3_full_covariates_plus_heart_mri")
                & (model_comparison_df["split"] == "test"),
                "cindex",
            ].iloc[0]
        ),
        ...
        "n_original_heart_features": int(len(heart_feature_cols)),
        ...
        "note": (
            "Primary score is heart_mri_mortality_risk_score from elastic-net Cox. "
            "Clock age/acceleration are post-hoc residualized transforms adjusted for "
            "all retained non-heart covariates. Heart-specific covariates include age, sex, BMI, "
            "diastolic/systolic blood pressure, and assessment center when available."
        ),
    }

    with open(outdir / "heart_mri_mortality_clock_performance.json", "w") as f:
        json.dump(performance, f, indent=2)

    print("Done.")
    print(f"Outputs written to: {outdir}")
    print("Main output files:")
    print(f"  {outdir / 'heart_mri_mortality_clock_predictions.tsv'}")
    print(f"  {outdir / 'heart_mri_mortality_clock_test_predictions.tsv'}")
    print(f"  {outdir / 'heart_mri_mortality_clock_coefficients.tsv'}")
    print(f"  {outdir / 'heart_mri_mortality_clock_nonzero_coefficients.tsv'}")
    print(f"  {outdir / 'heart_mri_mortality_clock_model_comparison.tsv'}")
    print(f"  {outdir / 'heart_mri_mortality_clock_incremental_value_delta_cindex.tsv'}")
    print(f"  {outdir / 'heart_mri_mortality_clock_model.joblib'}")
    print(f"  {outdir / 'heart_mri_mortality_clock_performance.json'}")
