# ============================================================
# All mortality clock performance visualization
...
# - MRI folders and files use lower-case organ names, e.g. brain_mri_mortality_clock.
# - Proteomics/metabolomics folders may start with capitalized organ/system names,
#   but output files are lower-case, e.g. Brain_proteomics_mortality_clock/
#   brain_proteomics_mortality_clock_performance.json.
# - We preserve the original per-clock output folders and file naming.
...
calc_cindex <- function(df) {
  ...
  # Larger mortality risk score means higher hazard, so use -risk_score.
...
# ============================================================
# 4. Per-clock processing function
# ============================================================
...
plot_one_clock <- function(meta_row, horizon_years = 5, n_boot = 500, skip_missing = TRUE) {
  ...
  message("\n============================================================")
  message("Processing ", meta$clock_label)
  ...
  accel_z_col <- paste0(feature_token, "_mortality_clock_acceleration_z")
  accel_year_col <- paste0(feature_token, "_mortality_clock_acceleration_years")
  clock_age_col <- paste0(feature_token, "_mortality_clock_age_years")

  # The generated omics outputs use age_at_baseline; MRI uses age_at_imaging.
  age_col <- first_existing_col(pred, c("age_at_imaging", "age_at_baseline"))
  if (is.na(age_col)) stop("Could not find age_at_imaging or age_at_baseline for ", meta$clock_label)

  if (!accel_z_col %in% colnames(pred)) {
    warning("Missing acceleration z column for ", meta$clock_label, "; using standardized risk score as fallback.")
  ...
  required_cols <- c("participant_id", "time_years", "event", "split", "sex", risk_score_col, accel_z_col)
  ...
  if (risk_col_requested %in% colnames(pred) && any(is.finite(pred[[risk_col_requested]]), na.rm = TRUE)) {
    ...
    if (length(usable_risk_cols) > 0) {
      ...
      warning("No usable absolute-risk column found for ", meta$clock_label, ". Calibration panel will be skipped.")
    }
  }
  ...
  # Preserve original per-clock output naming.
  out_pdf <- file.path(meta$clock_dir, paste0(meta$prefix, "_model_performance.pdf"))
  out_png <- file.path(meta$clock_dir, paste0(meta$prefix, "_model_performance.png"))
  out_overfit <- file.path(meta$clock_dir, paste0(meta$prefix, "_overfitting_summary.tsv"))
  out_quartile <- file.path(meta$clock_dir, paste0(meta$prefix, "_risk_quartile_summary.tsv"))
  ...
  if (length(unique(risk_breaks)) < length(risk_breaks)) {
    warning("Non-unique training risk quartile breaks for ", meta$clock_label, ". Using within-split quartiles.")
    pred <- pred %>%
      group_by(split) %>%
      mutate(
  ...
  if (!is.null(risk_col)) {
    ...
    if (nrow(cal_input) > 0) {
      ...
      out_cal <- file.path(meta$clock_dir, paste0(meta$prefix, "_calibration_", local_horizon_years, "y.tsv"))
  ...
  if (sum(test_for_km$event, na.rm = TRUE) >= 2) {
    ...
    km_df <- tibble()
    warning("Too few test-set events for Kaplan-Meier by quartile for ", meta$clock_label, ".")
  }
  ...
  p_density <- pred %>%
    ...
    labs(title = "C. Risk-score distribution", subtitle = "Train, validation, and test distributions should broadly overlap", x = glue("{meta$clock_label} mortality risk score"), y = "Density") +
    theme_clock()
  ...
  if (nrow(km_df) > 0) {
    p_km <- km_df %>%
      ...
      labs(title = "D. Test-set mortality separation", subtitle = "Kaplan-Meier cumulative mortality by training-defined risk quartile", x = glue("Years after {time_origin_label}"), y = "Cumulative mortality") +
      theme_clock()
  } else {
    p_km <- ggplot() +
  ...
  final_fig <-
    ...
    (p_quartile | p_cal) +
    plot_annotation(
      title = glue("{meta$clock_label} mortality clock performance"),
...
# ============================================================
# 5. Loop over all mortality clocks
# ============================================================
...
if (length(results) == 0) stop("No mortality clocks were successfully processed.")

# ============================================================
# 6. Combined cross-clock outputs
# ============================================================
...
out_combined_overfit <- file.path(combined_outdir, "all_mortality_clock_overfitting_summary.tsv")
out_combined_quartile <- file.path(combined_outdir, "all_mortality_clock_risk_quartile_summary.tsv")
out_combined_calibration <- file.path(combined_outdir, "all_mortality_clock_calibration_summary.tsv")
...
message("\n============================================================")
message("Writing combined all-clock outputs")
message("============================================================")
...
# ============================================================
# 7. Combined figure: discrimination, overfitting, and M3-M1
# ============================================================

# Order clocks by modality and test C-index within modality.
...
p_cindex_heat <- combined_cindex_plot_tbl %>%
  ...
  geom_text(aes(label = sprintf("%.3f", cindex)), size = 3.0, color = "#17202A") +
  scale_fill_gradient(low = "#EAF2F8", high = "#1F618D", limits = c(0.45, NA), oob = scales::squish) +
  labs(
...
# B. Optimism gap: train-test and validation-test.
optimism_tbl <- combined_summary_plot_tbl %>%
  select(clock_id, clock_plot, modality, train_minus_test, validation_minus_test) %>%
  pivot_longer(
  ...
  mutate(
    ...
    comparison = factor(comparison, levels = c("Train - test", "Validation - test"))
  )
...
p_optimism <- optimism_tbl %>%
  ...
  geom_point(size = 2.0, alpha = 0.9, position = position_dodge(width = 0.55)) +
  scale_color_manual(values = c("Train - test" = "#2E86AB", "Validation - test" = "#F18F01")) +
  labs(
...
# D. Compact quality map: generalization versus incremental value.
base_quality <- combined_summary_plot_tbl %>%
  ...
  geom_point(aes(size = cindex_test, shape = delta_significant), alpha = 0.9) +
...
if (requireNamespace("ggrepel", quietly = TRUE)) {
  ...
  p_quality <- base_quality +
    geom_text(aes(label = organ_label), size = 2.4, check_overlap = TRUE, vjust = -0.7, show.legend = FALSE)
}
...
combined_fig <-
  ...
  plot_annotation(
    ...
    caption = "M1 = covariate baseline. M3 = covariates plus imaging/protein/metabolite features. Missing clocks are skipped, e.g. a still-running Metabolic metabolomics clock.",
    theme = theme(
...
message("  - Per-clock PDF/PNG figures and original-format summary TSVs are saved in each clock folder.")
