#!/usr/bin/env python3
"""
Compare held-out test-set mortality clocks versus matched chronological-age BAGs for incident disease onset using disease-free controls.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Test-only Cox comparison of mortality clocks versus BAGs for incident disease onset using disease-free controls."
    )
    parser.add_argument("--icd_tsv", required=True, type=str)
    parser.add_argument("--output_tsv", required=True, type=str)
    parser.add_argument(
        "--test_predictions_root",
        default="/cbica/home/wenju/Reproducibile_paper/WholeBodyClock",
        type=str,
        help=(
            "Root containing one <clock>_mortality_clock directory per EPOCH clock, "
            "with a *_mortality_clock_test_predictions.tsv file inside."
        ),
    )
    parser.add_argument(
        "--bag_tsv",
...
def disease_id_from_path(path: str) -> str:
    ...
    for suffix in ("_diagnosis_clock_disease_free.tsv", "_diagnosis_clock.tsv"):
...
def discover_test_prediction_files(root: str) -> Dict[str, str]:
    """Map normalized clock names to held-out test prediction files."""
    root_path = Path(root)
    if not root_path.exists():
        raise FileNotFoundError(f"Test-prediction root does not exist: {root}")

    suffix = "_mortality_clock_test_predictions.tsv"
    files = sorted(root_path.glob("*mortality_clock/*_mortality_clock_test_predictions.tsv"))
    mapping: Dict[str, str] = {}
    for path in files:
        name = path.name
        if not name.endswith(suffix):
            continue
        clock_name = name[: -len(suffix)]
        key = clock_name.lower()
        if key in mapping and Path(mapping[key]).resolve() != path.resolve():
            raise ValueError(
                f"Multiple test prediction files found for clock {clock_name}: "
...
def read_test_clock_predictions(
    ...
    if test_path is None:
        expected = os.path.join(
            ...
            f"{clock}_mortality_clock_test_predictions.tsv",
        )
...
def construct_survival_data(args) -> Tuple[pd.DataFrame, str]:
    ...
    if not os.path.basename(args.icd_tsv).endswith("_diagnosis_clock_disease_free.tsv"):
        warnings.warn(
            "Input filename does not end with _diagnosis_clock_disease_free.tsv. "
            "Confirm that non-cases are the intended disease-free CN controls."
        )
    # The ICD file supplies the disease-free case/CN population, disease status/date,
    # and participant IDs. Mortality EPOCH scores are loaded separately from each
    # test-only analysis. Mortality EPOCH scores are loaded separately from each
    # clock's held-out test prediction file inside analyze_pair().
    required_clock_cols = ["participant_id", "case", "date"]
    missing_clock = [c for c in required_clock_cols if c not in df_clock_all.columns]
    if missing_clock:
        raise ValueError(f"Missing columns in ICD TSV {args.icd_tsv}: {missing_clock}")
...
def empty_result(disease_id: str, pair: Dict[str, str], status: str, error: str = "") -> Dict[str, object]:
    ...
    numeric_cols = [
        "N_test_prediction_available", "N_test_merged_before_complete_case",
...
def analyze_pair(
    data: pd.DataFrame, disease_id: str, pair: Dict[str, str], args, file_map: Dict[str, str]
) -> Dict[str, object]:
    clock = pair["clock"]
    bag = pair["bag"]
    ...
    if bag not in data.columns:
        return empty_result(disease_id, pair, "missing_columns", f"Missing {bag}")

    try:
        test_pred, test_file, test_score_col = read_test_clock_predictions(args, pair, file_map)
    except Exception as exc:
        return empty_result(disease_id, pair, "test_predictions_failed", str(exc))

    # Preserve the disease-free input population: cases and predefined disease-free
    # CN controls are not redefined here. The only population restriction is the
    # inner join to this clock's untouched mortality-EPOCH test set.
    pair_data = data.merge(test_pred, on="participant_id", how="inner", validate="many_to_one")
    ...
    covars_initial = base_covariates(pair_data, age_col)
    needed = [clock, bag, time_col, "case"] + covars_initial
    df = pair_data[needed].copy()
    for c in needed:
    ...
    if not standardize_inplace(df, clock, "clock_z"):
        out = empty_result(disease_id, pair, "zero_variance_clock")
        out.update(test_metadata)
        out.update({"N": len(df), "N_case": n_case, "N_noncase": n_noncase})
        out.update(followup_summary)
        return out
    if not standardize_inplace(df, bag, "bag_z"):
        out = empty_result(disease_id, pair, "zero_variance_bag")
        out.update(test_metadata)
        out.update({"N": len(df), "N_case": n_case, "N_noncase": n_noncase})
        out.update(followup_summary)
        return out
...
def main():
    ...
    file_map = discover_test_prediction_files(args.test_predictions_root)
    print(f"Discovered {len(file_map)} held-out mortality-clock test prediction files.")

    rows = []
    for pair in CLOCK_BAG_PAIRS:
        rows.append(analyze_pair(data, disease_id, pair, args, file_map))
