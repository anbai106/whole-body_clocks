...
# TEST-ONLY runner for one ICD endpoint: matched BAG cumulative survival analysis.
# Usage:
...
# Leave TEST_SPLIT_COL empty to let Python auto-detect the split column.
...
ARGS=(
    ...
    --test_label "${TEST_SPLIT_VALUE}"
)

if [[ -n "${TEST_SPLIT_COL}" ]]; then
    ARGS+=(--split_col "${TEST_SPLIT_COL}")
fi
...
export CV_FOLDS RANK_SOURCE PAIR_COMPLETE_CASE_WITH_EPOCH BAG_TSV TEST_SPLIT_COL TEST_SPLIT_VALUE
