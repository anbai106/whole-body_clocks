#!/usr/bin/env python3
"""
STEP 3B: TEST-ONLY cumulative Cox survival analysis for the matched proteomics
and metabolomics BAGs corresponding to the mortality EPOCH clocks.

IMPORTANT TEST-ONLY BEHAVIOR
----------------------------
All participant-level cumulative survival analyses are restricted BEFORE
survival construction, complete-case filtering, score standardization, Cox
fitting, and cross-validation to rows belonging to the held-out test split.

The split is taken from the disease-specific ICD/EPOCH TSV so the BAG analysis
uses the same held-out participants as the EPOCH analysis. The split column is
auto-detected from common names (split, data_split, dataset_split, partition,
set) unless TEST_SPLIT_COL or --split_col is supplied. The test label defaults
to "test" and can be changed with TEST_SPLIT_VALUE or --test_label.

The BAG ranking is still read from --individual_result_tsv. For a completely
held-out workflow, point that argument to the prespecified ranking source you
intend to use. The participant-level Cox analyses themselves use test
participants only.
"""
...
# -----------------------------------------------------------------------------
# TEST-ONLY analysis controls.
# Leave TEST_SPLIT_COL empty to auto-detect a split column from the ICD TSV.
# -----------------------------------------------------------------------------
TEST_SPLIT_COL = os.getenv("TEST_SPLIT_COL", "").strip()
TEST_SPLIT_VALUE = os.getenv("TEST_SPLIT_VALUE", "test").strip()
TEST_SPLIT_CANDIDATES = ("split", "data_split", "dataset_split", "partition", "set")
ANALYSIS_SUBSET = "test_only"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--split_col",
        default=TEST_SPLIT_COL,
        help=(
            "Participant split column in the disease-specific ICD TSV. "
            "Default: TEST_SPLIT_COL environment variable; if empty, auto-detect."
        ),
    )
    p.add_argument(
        "--test_label",
        default=TEST_SPLIT_VALUE,
        help="Value identifying held-out test rows in the split column (default: test).",
    )
...
def resolve_split_column(df: pd.DataFrame, requested: str) -> str:
    """Resolve the participant-level split column used for mandatory test filtering."""
    if requested:
        if requested not in df.columns:
            raise ValueError(
                f"Requested split column '{requested}' is not present in the ICD TSV. "
    ...
    raise ValueError(
        "TEST-ONLY analysis requires a participant split column in the ICD TSV. "
        "Could not auto-detect one of: "
        + ", ".join(TEST_SPLIT_CANDIDATES)
        + ". Set TEST_SPLIT_COL or pass --split_col explicitly."
    )
...
def filter_to_test_split(
    ...
    """Restrict to the held-out test split using case-insensitive string matching."""
    values = df[split_col].astype(str).str.strip().str.lower()
    target = str(test_label).strip().lower()
    keep = values == target
    out = df.loc[keep].copy()

    if out.empty:
        observed = sorted(df[split_col].dropna().astype(str).str.strip().unique().tolist())
        raise ValueError(
            f"No rows matched test label '{test_label}' in split column '{split_col}'. "
            f"Observed values (up to 20): {observed[:20]}"
        )

    print(
        f"TEST-ONLY filter: {split_col} == {test_label!r}: "
        f"{len(out):,}/{len(df):,} rows retained.",
        flush=True,
    )
    return out
...
def construct_survival_data(args: argparse.Namespace) -> Tuple[pd.DataFrame, str, pd.DataFrame]:
    ...
    n_icd_all_splits = len(df_icd_all)
    split_col = resolve_split_column(df_icd_all, args.split_col)
    df_icd_all = filter_to_test_split(df_icd_all, split_col, args.test_label)
    n_icd_test_split = len(df_icd_all)
    ...
    audit_rows.append({
        "disease_id": disease_id,
        "step": "icd_input_all_splits",
        "N": int(n_icd_all_splits),
        "N_case": np.nan,
        "N_noncase": np.nan,
    })
    audit_rows.append({
        "disease_id": disease_id,
        "step": f"after_test_split_filter__{split_col}={args.test_label}",
        "N": int(n_icd_test_split),
        "N_case": int(pd.to_numeric(df_icd.get("case"), errors="coerce").fillna(0).sum()),
        "N_noncase": int((pd.to_numeric(df_icd.get("case"), errors="coerce").fillna(0) == 0).sum()),
    })
    add_audit("icd_input_test_only", df_icd)
    ...
    print(
        f"TEST-ONLY analysis population for {disease_id}: N={len(analysis):,}, "
...
def build_score_ranking(args: argparse.Namespace, disease_id: str) -> pd.DataFrame:
    ...
    ranked["analysis_subset"] = ANALYSIS_SUBSET
    ranked["test_label"] = args.test_label
    return ranked
...
def common_result_columns(
    ...
    return {
        ...
        "split_column": args.split_col if args.split_col else "auto_detected_from_icd_tsv",
        "test_label": args.test_label,
        "analysis_clock_set": "11_proteomics_plus_4_metabolomics_BAGs",
        "complete_case_mode": "test_only__all_15_BAGs_and_all_15_EPOCHs_for_all_steps" if PAIR_COMPLETE_CASE_WITH_EPOCH else "test_only__all_15_BAGs_for_all_steps",
        "score_type": "BAG",
