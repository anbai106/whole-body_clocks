...
#SBATCH --job-name=SA_cum_BAG_test
#SBATCH --array=0-3572
#SBATCH --mem-per-cpu=12G
#SBATCH --time=0-05:59:00
#SBATCH --output=/cbica/home/wenju/output/SA_cum_BAG_test_%A_%a.out
#SBATCH --error=/cbica/home/wenju/output/SA_cum_BAG_test_%A_%a.err

set -euo pipefail

# STEP 3B TEST-ONLY cumulative survival analysis for the matched BAGs corresponding to
...
OUT_DIR="${OUT_DIR:-/cbica/home/wenju/Reproducibile_paper/WholeBodyClock/mortality_clock/SA/output_cumulative_BAG_PM/disease_free_test_only_cv}"
RUNNER="${RUNNER:-/cbica/home/wenju/Project/whole-body_clocks/mortality_clock/survival_analysis/ICD10/disease_free/cumulative_epoch_survival_pipeline/testdata_only/run_3_cumulative_bag_survival_test_only.sh}"
PYTHON_SCRIPT="${PYTHON_SCRIPT:-/cbica/home/wenju/Project/whole-body_clocks/mortality_clock/survival_analysis/ICD10/disease_free/cumulative_epoch_survival_pipeline/testdata_only/survival_analysis_cumulative_bag_test_only.py}"
...
# Mandatory held-out test split controls.
# Leave empty for auto-detection among split/data_split/dataset_split/partition/set.
TEST_SPLIT_COL="${TEST_SPLIT_COL:-}"
...
OUTPUT_TSV="${OUT_DIR}/cox_cumulative_BAG_PM_test_only_${ICD}.tsv"
RANK_TSV="${OUT_DIR}/rank_order_BAG_PM_test_only_${ICD}.tsv"
AUDIT_TSV="${OUT_DIR}/audit_BAG_PM_test_only_${ICD}.tsv"
...
echo "Analysis subset: held-out TEST only"
echo "Test split column: ${TEST_SPLIT_COL:-AUTO-DETECT}"
echo "Test split value: ${TEST_SPLIT_VALUE}"

export PYTHON_SCRIPT BAG_TSV RANK_P_THRESHOLD RANK_SORT_MODE NON_SIG_ORDER RANK_SOURCE PAIR_COMPLETE_CASE_WITH_EPOCH CV_FOLDS MIN_CASE MIN_NONCASE PENALIZER COVARIATE_SET ADMIN_CENSOR_DATE OVERWRITE TEST_SPLIT_COL TEST_SPLIT_VALUE
