#!/usr/bin/env python3
"""
STEP 3: TEST-ONLY cumulative Cox survival analysis for proteomics and
metabolomics mortality EPOCH clocks predicting incident disease onset.
...
The split column is auto-detected from common names (split, data_split,
dataset_split, partition, set) unless TEST_SPLIT_COL or --split_col is supplied.
The test label defaults to "test" and can be changed with TEST_SPLIT_VALUE or
--test_label.

The disease-specific clock ranking is still read from --individual_result_tsv.
For a completely held-out workflow, point that argument to the prespecified
ranking source you intend to use (for example, training/validation-derived
individual results). The participant-level Cox analyses themselves use test
participants only.
"""
...
def construct_survival_data(args: argparse.Namespace) -> Tuple[pd.DataFrame, str, pd.DataFrame]:
    ...
    n_icd_all_splits = len(df_clock_all)
    split_col = resolve_split_column(df_clock_all, args.split_col)
    df_clock_all = filter_to_test_split(df_clock_all, split_col, args.test_label)
    n_icd_test_split = len(df_clock_all)
    ...
    audit_rows.append({
        ...
        "N_case": int(pd.to_numeric(df_clock.get("case"), errors="coerce").fillna(0).sum()),
        "N_noncase": int((pd.to_numeric(df_clock.get("case"), errors="coerce").fillna(0) == 0).sum()),
    })
    add_audit("clock_input_test_only", df_clock)
...
def common_result_columns(
    ...
    return {
        ...
        "complete_case_mode": "test_only__all_15_clocks_for_all_steps",
    }
