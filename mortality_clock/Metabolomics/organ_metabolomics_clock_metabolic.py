#!/usr/bin/env python3
"""
General organ metabolomics mortality clock using elastic-net Cox survival modeling.
...
Output naming is controlled by --organ, e.g. --organ heart creates:
  heart_metabolomics_mortality_clock_predictions.tsv
  heart_metabolomics_mortality_clock_performance.json
  etc.
"""
...
def parse_args():
    ...
    p.add_argument("--death-xlsx", default="/cbica/home/wenju/Dataset/UKBB_UMelbourne/Death_related_var_from_Ye.xlsx")
    ...
    p.add_argument("--l1-ratios", default="0.05,0.1,0.25,0.5,0.75,1.0")
    ...
    p.add_argument(
        ...
        default=0.01,
        help=(
            "Minimum alpha ratio for Coxnet. Larger values keep stronger regularization "
            "and reduce numerical overflow risk. If Coxnet still reports that weights "
            "are too large, try 0.05 or 0.10."
        ),
    )
    p.add_argument(
        ...
        default=0.05,
        help=(
            "Penalty factor for non-omics covariates. Use a small nonzero value, "
            "rather than exactly 0, to avoid numerical instability from fully "
...
def output_prefix(organ):
    return f"{organ}_metabolomics_mortality_clock"
...
def construct_survival_dataset(df):
    df = df.copy()
    df["death_before_or_on_sample"] = df["death_date"].notna() & df["sample_date"].notna() & (df["death_date"] <= df["sample_date"])
    ...
    df["event"] = df["death_date"].notna() & (df["death_date"] > df["sample_date"]) & (df["death_date"] <= df["admin_censor_date"])
...
def print_design_matrix_diagnostics(X, y, label):
    """
    Print basic finite/range diagnostics after preprocessing.

    This is useful for metabolomics Coxnet failures such as:
      ArithmeticError: Numerical error, because weights are too large.
    """
    event_count = int(np.sum(y["event"]))
    print(f"{label} design-matrix diagnostics:")
    print(f"  shape: {X.shape}")
    print(f"  events: {event_count}")
    print(f"  all finite: {bool(np.isfinite(X).all())}")
    if X.size > 0:
        print(f"  max abs value: {float(np.nanmax(np.abs(X))):.4f}")
...
def fit_and_select_coxnet(
    ...
    Metabolomics features are assigned penalty_factor=1. Non-omics covariates
    are assigned a small nonzero penalty by default. This is more numerically
    stable than leaving covariates completely unpenalized, especially for broad,
    highly correlated metabolomics panels.

    alpha_min_ratio is set to a conservative numeric value by default rather
    than "auto", because very small alpha values can produce extreme Cox risk
    weights and trigger scikit-survival's "weights are too large" error.
    """
    print_design_matrix_diagnostics(X_train, y_train, "Training")
    print_design_matrix_diagnostics(X_val, y_val, "Validation")

    if alpha_min_ratio <= 0 or alpha_min_ratio >= 1:
        raise ValueError("--alpha-min-ratio must be > 0 and < 1.")

    if covariate_penalty_factor < 0:
        raise ValueError("--covariate-penalty-factor must be >= 0.")

    penalty_factor = np.ones(len(feature_names), dtype=float)
    is_organ = organ_feature_mask(feature_names, organ_feature_cols)

    # The omics features are penalized normally; non-omics covariates are lightly
    # penalized instead of completely unpenalized to prevent numerical overflow.
    penalty_factor[~is_organ] = covariate_penalty_factor
    ...
    for l1_ratio in l1_ratios:
        print(
            f"Fitting Coxnet path for l1_ratio={l1_ratio}, "
            f"alpha_min_ratio={alpha_min_ratio}, "
            f"covariate_penalty_factor={covariate_penalty_factor}"
        )
        ...
        try:
            try:
                ...
                warnings.warn(
                    ...
                    "Covariates will be penalized together with metabolomics features."
                )
                model = CoxnetSurvivalAnalysis(
                    l1_ratio=l1_ratio,
                    n_alphas=n_alphas,
            ...
            failed_paths.append((float(l1_ratio), str(exc)))
            warnings.warn(
                f"Coxnet numerical failure for l1_ratio={l1_ratio}: {exc}. "
                "Skipping this path. If all paths fail, increase --alpha-min-ratio "
                "to 0.05 or 0.10."
            )
            continue
            ...
            failed_paths.append((float(l1_ratio), str(exc)))
            warnings.warn(
                f"Coxnet failed for l1_ratio={l1_ratio}: {exc}. "
                "Skipping this path."
            )
            continue
    ...
    best["failed_paths"] = failed_paths

    if best["alpha"] is None:
        raise RuntimeError(
            "Failed to select a Coxnet model because all regularization paths failed. "
            "This usually means the path explored alpha values that were too weak for "
            "the metabolomics feature distribution. Try rerunning with "
            "--alpha-min-ratio 0.05 or --alpha-min-ratio 0.10, and consider "
            "--l1-ratios 0.05,0.1,0.25,0.5."
        )
...
def fit_final_model(X_trainval, y_trainval, best, penalty_factor):
    """
    Refit final Coxnet model on train+validation using the selected l1_ratio and alpha.

    In rare cases, fit_baseline_model=True can trigger the same numerical
    instability seen during path fitting. If that happens, we retry with a
    modestly larger alpha and record the inflation factor in best.
    """
    alpha0 = float(best["alpha"])
    alpha_factors = [1.0, 2.0, 5.0, 10.0, 20.0, 50.0, 100.0]
    last_exc = None

    for factor in alpha_factors:
        alpha_try = alpha0 * factor
        try:
            try:
                model = CoxnetSurvivalAnalysis(
            ...
            if factor > 1.0:
                warnings.warn(
                    f"Final Coxnet fit required stronger regularization: "
                    f"alpha {alpha0} -> {alpha_try} (factor {factor})."
                )
                best["alpha_original_from_validation"] = alpha0
                best["alpha"] = float(alpha_try)
                best["alpha_inflation_for_final_fit"] = float(factor)
            else:
                best["alpha_inflation_for_final_fit"] = 1.0
            ...
            last_exc = exc
            warnings.warn(
                f"Final Coxnet fit failed with alpha={alpha_try}: {exc}. "
                "Retrying with stronger regularization."
            )
        except Exception as exc:
            last_exc = exc
            warnings.warn(
    ...
    raise RuntimeError(
        "Final Coxnet model failed even after increasing alpha. "
...
def main():
    ...
    print(f"Building {organ} metabolomics mortality clock")
    ...
    print("Final prospective baseline-omics mortality dataset:")
    print(f"  N = {df.shape[0]}")
    print(f"  Deaths after baseline = {int(df['event'].sum())}")
    print(f"  Censored = {int((~df['event']).sum())}")
    print(f"  Median follow-up years = {df['time_years'].median():.2f}")
    if df["event"].sum() < 20:
        warnings.warn("Very few mortality events. The fitted clock may be unstable.")

    dataset_cols = ["participant_id", "baseline_date", "sample_date", "death_date", "admin_censor_date", "end_date", "time_days", "time_years", "event", "age_at_baseline", "sex"]
    ...
    def make_pred_frame(part, split, risk):
        base = ["participant_id", "sample_date", "death_date", "admin_censor_date", "end_date", "time_years", "event", "age_at_baseline", "sex"]
