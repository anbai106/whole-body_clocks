#!/usr/bin/env python3
"""
Build a brain MRI stroke L'EPOCH clock in UK Biobank using elastic-net Cox survival modeling.

Time zero:
  UKB imaging assessment date, field 53-2.0

Event:
  incident stroke date, field 42006-0.0, after imaging and before censoring
...
Primary output:
  brain_mri_stroke_risk_score from the survival model.

Recommended downstream phenotype:
  brain_mri_stroke_clock_acceleration_z, the residualized risk score after adjusting for retained non-brain covariates in the training set.
"""
...
PREFIX = "brain_mri_stroke_clock"
RISK_COL = "brain_mri_stroke_risk_score"
ACCEL_Z_COL = "brain_mri_stroke_clock_acceleration_z"
ACCEL_YEARS_COL = "brain_mri_stroke_clock_acceleration_years"
CLOCK_AGE_COL = "brain_mri_stroke_clock_age_years"


def parse_args():
    parser = argparse.ArgumentParser(description="Build brain MRI stroke L'EPOCH clock.")
    parser.add_argument(
        "--stroke-xlsx",
        default="/cbica/home/wenju/Dataset/UKBB_UMelbourne/Death_related_var_from_Ye.xlsx",
        help="Excel file containing UKB assessment dates, stroke date, and optionally death date.",
    )
    parser.add_argument(
        "--id-match-csv",
    ...
    parser.add_argument(
        "--admin-censor-date",
        required=True,
        help="Endpoint-specific administrative censor date for stroke ascertainment, e.g. 2022-11-30.",
    )
...
def load_stroke_data(stroke_xlsx, id_match_csv):
    ...
    if "40000-0.0" in d.columns:
        ...
        warnings.warn(
            "Death date field 40000-0.0 was not found. stroke-free participants "
...
def construct_survival_dataset(df):
    ...
    Exclude prevalent stroke before/on imaging.
    Event is incident stroke after imaging and before censoring.
...
def main():
    ...
    print("Merging data...")
    df = brain.merge(stroke, on="participant_id", how="inner")
    if cov is not None:
        df = df.merge(cov, on="participant_id", how="left", suffixes=("", "_cov"))

    print("Constructing prospective stroke survival outcome...")
    ...
    if df["event"].sum() < 20:
        warnings.warn("Very few stroke events. The fitted brain MRI stroke clock may be unstable.")

    keep_dataset_cols = ["participant_id", "baseline_date", "imaging_date", "stroke_date", "death_date", "admin_censor_date", "censor_date", "end_date", "time_days", "time_years", "event", "age_at_imaging", "sex"]
    if "DLICV" in df.columns:
    ...
    def make_pred_frame(part, split, risk):
        base = ["participant_id", "imaging_date", "stroke_date", "death_date", "admin_censor_date", "censor_date", "end_date", "time_years", "event", "age_at_imaging", "sex"]
    ...
    print("Adding residualized stroke-clock acceleration...")
    ...
    performance = {
        ...
        "event_date": "UKB stroke date, field 42006-0.0",
        "censoring": "earlier of death date field 40000-0.0 and endpoint-specific administrative censor date",
        "note": "Primary score is brain_mri_stroke_risk_score from elastic-net Cox. Acceleration z-score is residualized on retained non-brain covariates and is recommended for downstream analyses.",
    }
