#!/usr/bin/env python3
"""
General organ MRI stroke clock using elastic-net Cox survival modeling.
...
Output naming is controlled by --organ, e.g. --organ heart creates:
  heart_mri_stroke_clock_predictions.tsv
  heart_mri_stroke_clock_performance.json
  etc.
"""
...
def parse_args():
    ...
    p.add_argument("--stroke-xlsx", default="/cbica/home/wenju/Dataset/UKBB_UMelbourne/Death_related_var_from_Ye.xlsx")
    ...
    p.add_argument("--imaging-session-id", default="ses-2", help="Use 'none' to disable filtering.")
    p.add_argument("--feature-start-column", default="diagnosis", help="All columns after this column are organ MRI features.")
...
def output_prefix(organ):
    return f"{organ}_mri_stroke_clock"


def load_stroke_data(stroke_xlsx, id_match_csv):
    d = pd.read_excel(stroke_xlsx)
    ...
    required = ["participant_id", "53-0.0", "53-2.0", "42006-0.0"]
    missing = [c for c in required if c not in d.columns]
    if missing:
        raise ValueError(f"stroke/assessment file is missing required columns: {missing}")

    keep = required.copy()

    # UKB death date is commonly field 40000-0.0.
    # Keep it if available so stroke-free participants can be censored at death.
    if "40000-0.0" in d.columns:
        keep.append("40000-0.0")
    else:
        warnings.warn(
            "Death date field 40000-0.0 was not found. "
            "stroke-free participants will be censored only at the administrative censor date."
        )
    ...
    d["stroke_date"] = pd.to_datetime(d["42006-0.0"], errors="coerce")
...
def load_organ_data(organ_tsv, organ, imaging_session_id):
    ...
    if imaging_session_id and str(imaging_session_id).lower() not in {"none", "null", ""}:
        if "session_id" in df.columns:
            ...
            warnings.warn("--imaging-session-id was provided, but session_id is missing.")
...
def infer_feature_columns(df_organ, feature_start_column):
    ...
    if not features:
        raise ValueError(f"No MRI features found after {feature_start_column}.")
...
def construct_survival_dataset(df):
    """
    Construct prospective stroke survival outcome using imaging date as time zero.

    Event:
        Incident stroke after imaging and before censoring.

    Exclusions:
        - Missing imaging date
        - stroke before or on imaging date
    ...
    df["stroke_before_or_on_imaging"] = (
        df["stroke_date"].notna()
        & df["imaging_date"].notna()
        & (df["stroke_date"] <= df["imaging_date"])
    )

    n_pre = int(df["stroke_before_or_on_imaging"].sum())
    if n_pre:
        warnings.warn(f"Excluding {n_pre} participants with stroke before/on imaging date.")

    df = df.loc[df["imaging_date"].notna()].copy()
    df = df.loc[~df["stroke_before_or_on_imaging"]].copy()
    ...
    # Incident stroke must occur after imaging and on/before censor date.
    df["event"] = (
        df["stroke_date"].notna()
        & (df["stroke_date"] > df["imaging_date"])
        & (df["stroke_date"] <= df["censor_date"])
    )

    df["end_date"] = df["censor_date"]
    df.loc[df["event"], "end_date"] = df.loc[df["event"], "stroke_date"]
...
def add_basic_covariates(df):
    ...
    if age_col in df.columns:
        ...
        warnings.warn("Using numeric diagnosis as age_at_imaging fallback. Please verify this is correct.")
        df["age_at_imaging"] = pd.to_numeric(df["diagnosis"], errors="coerce")
    else:
        raise ValueError("Could not infer age_at_imaging.")
    ...
    df = mean_existing_numeric_columns(df, ["diastolic_blood_pressure_automated_reading_f4079_2_0", "diastolic_blood_pressure_automated_reading_f4079_2_1"], "diastolic_bp_at_imaging")
    df = mean_existing_numeric_columns(df, ["systolic_blood_pressure_automated_reading_f4080_2_0", "systolic_blood_pressure_automated_reading_f4080_2_1"], "systolic_bp_at_imaging")
    if "uk_biobank_assessment_centre_f54_2_0" in df.columns:
        df["uk_biobank_assessment_centre_f54_2_0"] = df["uk_biobank_assessment_centre_f54_2_0"].astype("category")
    return df


def build_design_matrix(df, organ_feature_cols):
    numeric_covariates = ["age_at_imaging"]
    for c in ["bmi_at_imaging", "diastolic_bp_at_imaging", "systolic_bp_at_imaging"]:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce")
    ...
    if "uk_biobank_assessment_centre_f54_2_0" in df.columns:
        df["uk_biobank_assessment_centre_f54_2_0"] = df["uk_biobank_assessment_centre_f54_2_0"].astype("category")
        categorical_covariates.append("uk_biobank_assessment_centre_f54_2_0")
...
def paired_bootstrap_delta_cindex(y, risk_full, risk_base, organ, n_boot, random_state):
    ...
    comparison = f"M3_full_covariates_plus_{organ}_mri_vs_M1_covariate_baseline"
    if boots.size == 0:
    ...
    if delta > 0 and lo > 0:
        interp = f"{organ} MRI improves test-set C-index beyond the covariate baseline."
    elif delta > 0:
        interp = f"{organ} MRI has positive delta C-index, but the bootstrap CI includes or approaches zero."
    else:
        interp = f"No evidence that {organ} MRI improves test-set C-index beyond the covariate baseline."
...
def run_incremental_value_analysis(X_train, X_val, X_test, X_trainval, y_train, y_val, y_test, y_trainval, feature_names, organ_feature_cols, organ, final_model, n_bootstrap, random_state):
    ...
    m2 = f"M2_{organ}_mri_only"
    m3 = f"M3_full_covariates_plus_{organ}_mri"
    idx = {
    ...
    labels = {"M0_age_sex": "Age + sex", "M1_covariate_baseline": "Covariate baseline", m2: f"{organ} MRI only", m3: f"Covariates + {organ} MRI"}
    rows = []
...
def add_clock_age_and_acceleration(pred_df, organ, covariate_cols):
    df = pred_df.copy()
    risk_col = f"{organ}_mri_stroke_risk_score"
    z_col = f"{organ}_mri_stroke_clock_acceleration_z"
    yrs_col = f"{organ}_mri_stroke_clock_acceleration_years"
    age_col = f"{organ}_mri_stroke_clock_age_years"
    ...
    if np.isfinite(beta_age) and abs(beta_age) > 1e-8:
        ...
        df[age_col] = df["age_at_imaging"] + df[yrs_col]
    else:
...
def main():
    ...
    print(f"Building {organ} MRI stroke clock")

    print("Loading stroke/assessment data...")
    stroke = load_stroke_data(args.stroke_xlsx, args.id_match_csv)
    stroke["admin_censor_date"] = admin_censor_date
    ...
    print("Merging data...")
    df = organ_df.merge(stroke, on="participant_id", how="inner")
    if cov is not None:
    ...
    print("Final prospective imaging stroke dataset:")
    print(f"  N = {df.shape[0]}")
    print(f"  Incident stroke events after imaging = {int(df['event'].sum())}")
    print(f"  Censored = {int((~df['event']).sum())}")
    print(f"  Median follow-up years = {df['time_years'].median():.2f}")
    if df["event"].sum() < 20:
        warnings.warn("Very few stroke events. The fitted clock may be unstable.")

    dataset_cols = ["participant_id", "baseline_date", "imaging_date", "stroke_date", "admin_censor_date", "end_date", "time_days", "time_years", "event", "age_at_imaging", "sex"]
    ...
    print(f"Running incremental-value analysis: does {organ} MRI add value beyond covariates?")
    ...
    def make_pred_frame(part, split, risk):
        base = ["participant_id", "imaging_date", "stroke_date", "admin_censor_date", "end_date", "time_years", "event", "age_at_imaging", "sex"]
    ...
    comparison_cols = {"M0_age_sex": "risk_score_M0_age_sex", "M1_covariate_baseline": "risk_score_M1_covariate_baseline", m2: f"risk_score_M2_{organ}_mri_only", m3: f"risk_score_M3_full_covariates_plus_{organ}_mri"}
    ...
    print("Adding approximate stroke-clock age and acceleration...")
    ...
    is_org = organ_feature_mask(feature_names, organ_feature_cols)
    coef_df = pd.DataFrame({"feature": feature_names, "coefficient": coef, "abs_coefficient": np.abs(coef), "penalty_factor": penalty_factor, "is_nonzero": coef != 0, f"is_{organ}_mri_feature": is_org}).sort_values("abs_coefficient", ascending=False)
