...
df_external = data.loc[data['Study'].isin(["ADNI_DOD", "AIBL", "BLSA", "OASIS", "PREVENT-AD"])]
# df_adni_test = df_adni.iloc[:1]
# df_adni_test.to_csv("~/test.tsv", sep="\t", index=False)
df_external.to_csv("/Users/hao/cubic-home/Reproducibile_paper/WholeBodyClock/adni_lepoch/external_5_studies_istaging.tsv", sep="\t", index=False)
print(df_external.columns.to_list())

#### SANITY CHECK FOR THE DX_Binary (v2.0 hasd issue for this)
# Standardize key variables
df_external["Date_parsed"] = pd.to_datetime(
    df_external["Date"],
    errors="coerce"
)

df_external["Age_numeric"] = pd.to_numeric(
    df_external["Age"],
    errors="coerce"
)

df_external["DX_Binary_clean"] = (
    df_external["DX_Binary"]
    .astype("string")
    .str.strip()
)
...
df_external.loc[
    df_external["DX_Binary_clean"].isin(
...
df_with_dx = df_external.loc[
    df_external["DX_Binary_clean"].notna()
].copy()
