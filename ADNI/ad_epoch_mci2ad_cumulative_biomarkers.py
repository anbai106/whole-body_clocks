#!/usr/bin/env python3
"""
Cumulative survival analysis for MCI-to-AD conversion in ADNI.
...
all variables required through M5. This guarantees paired comparisons across
...
DEFAULT_SPARE = ["SPARE_BA", "SPARE_AD"]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Cumulative MCI-to-AD survival analysis with CSF, SPARE, and AD EPOCH."
    )
    ...
    p.add_argument("--spare-ba-col", default="SPARE_BA")
    p.add_argument("--spare-ad-col", default="SPARE_AD")
...
def prepare_dataset(args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    baseline_requested = baseline_cols + [args.spare_ba_col, args.spare_ad_col]
    ...
    for col in ["Age", args.spare_ba_col, args.spare_ad_col] + csf_cols:
...
def make_specs(args: argparse.Namespace) -> list[ModelSpec]:
    ...
    m1 = m0 + abeta
    m2 = m1 + tau
    m3 = m2 + (args.spare_ba_col,)
    m4 = m3 + (args.spare_ad_col,)
    m5 = m4 + (args.epoch_col,)

    return [
        ModelSpec(0, "M0_age_sex", "baseline", m0),
        ModelSpec(1, "M1_plus_amyloid", "Abeta_CSF", m1),
        ModelSpec(2, "M2_plus_tau", "Tau_CSF+PTau_CSF", m2),
        ModelSpec(3, "M3_plus_SPARE_BA", args.spare_ba_col, m3),
        ModelSpec(4, "M4_plus_SPARE_AD", args.spare_ad_col, m4),
        ModelSpec(5, "M5_plus_AD_EPOCH", args.epoch_col, m5),
    ]
...
def main() -> None:
    ...
    log("============================================================")
    log("ADNI MCI-to-AD cumulative biomarker survival analysis")
