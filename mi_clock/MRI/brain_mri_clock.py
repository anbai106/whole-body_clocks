#!/usr/bin/env python3
"""
Build a brain MRI mi L'EPOCH clock in UK Biobank using elastic-net Cox survival modeling.

Time zero:
  UKB imaging assessment date, field 53-2.0

Event:
  incident mi date, field 42000-0.0, after imaging and before censoring
...
Primary output:
  brain_mri_mi_risk_score from the survival model.

Recommended downstream phenotype:
  brain_mri_mi_clock_acceleration_z, the residualized risk score after adjusting for retained non-brain covariates in the training set.
"""
...
PREFIX = "brain_mri_mi_clock"
RISK_COL = "brain_mri_mi_risk_score"
ACCEL_Z_COL = "brain_mri_mi_clock_acceleration_z"
ACCEL_YEARS_COL = "brain_mri_mi_clock_acceleration_years"
CLOCK_AGE_COL = "brain_mri_mi_clock_age_years"


def parse_args():
    parser = argparse.ArgumentParser(description="Build brain MRI mi L'EPOCH clock.")
    parser.add_argument(
        "--mi-xlsx",
        default="/cbica/home/wenju/Dataset/UKBB_UMelbourne/Death_related_var_from_Ye.xlsx",
        help="Excel file containing UKB assessment dates, mi date, and optionally death date.",
    )
    parser.add_argument(
        "--id-match-csv",
    ...
    parser.add_argument(
        "--admin-censor-date",
        required=True,
        help="Endpoint-specific administrative censor date for mi ascertainment, e.g. 2022-11-30.",
    )
...
def load_mi_data(mi_xlsx, id_match_csv):
    ...
    if "40000-0.0" in d.columns:
        ...
        warnings.warn(
            "Death date field 40000-0.0 was not found. mi-free participants "
...
def construct_survival_dataset(df):
    ...
    Exclude prevalent mi before/on imaging.
    Event is incident mi after imaging and before censoring.
...
def main():
    ...
    print("Merging data...")
    df = brain.merge(mi, on="participant_id", how="inner")
    if cov is not None:
        df = df.merge(cov, on="participant_id", how="left", suffixes=("", "_cov"))

    print("Constructing prospective mi survival outcome...")
    ...
    if df["event"].sum() < 20:
        warnings.warn("Very few mi events. The fitted brain MRI mi clock may be unstable.")

    keep_dataset_cols = ["participant_id", "baseline_date", "imaging_date", "mi_date", "death_date", "admin_censor_date", "censor_date", "end_date", "time_days", "time_years", "event", "age_at_imaging", "sex"]
    if "DLICV" in df.columns:
    ...
    def make_pred_frame(part, split, risk):
        base = ["participant_id", "imaging_date", "mi_date", "death_date", "admin_censor_date", "censor_date", "end_date", "time_years", "event", "age_at_imaging", "sex"]
    ...
    print("Adding residualized mi-clock acceleration...")
    ...
    performance = {
        ...
        "event_date": "UKB mi date, field 42000-0.0",
        "censoring": "earlier of death date field 40000-0.0 and endpoint-specific administrative censor date",
        "note": "Primary score is brain_mri_mi_risk_score from elastic-net Cox. Acceleration z-score is residualized on retained non-brain covariates and is recommended for downstream analyses.",
    }
