#!/usr/bin/env python3
"""
General organ MRI mi clock using elastic-net Cox survival modeling.
...
Output naming is controlled by --organ, e.g. --organ heart creates:
  heart_mri_mi_clock_predictions.tsv
  heart_mri_mi_clock_performance.json
  etc.
"""
...
def load_mi_data(mi_xlsx, id_match_csv):
    ...
    required = ["participant_id", "53-0.0", "53-2.0", "42000-0.0"]
...
def construct_survival_dataset(df):
    """
    Construct prospective mi survival outcome using imaging date as time zero.

    Event:
        Incident mi after imaging and before censoring.

    Exclusions:
        - Missing imaging date
        - mi before or on imaging date
    ...
    df["mi_before_or_on_imaging"] = (
        df["mi_date"].notna()
        & df["imaging_date"].notna()
        & (df["mi_date"] <= df["imaging_date"])
    )

    n_pre = int(df["mi_before_or_on_imaging"].sum())
    if n_pre:
        warnings.warn(f"Excluding {n_pre} participants with mi before/on imaging date.")

    df = df.loc[df["imaging_date"].notna()].copy()
    df = df.loc[~df["mi_before_or_on_imaging"]].copy()
    ...
    # Incident mi must occur after imaging and on/before censor date.
    df["event"] = (
        df["mi_date"].notna()
        & (df["mi_date"] > df["imaging_date"])
...
def add_clock_age_and_acceleration(pred_df, organ, covariate_cols):
    df = pred_df.copy()
    risk_col = f"{organ}_mri_mi_risk_score"
    z_col = f"{organ}_mri_mi_clock_acceleration_z"
    yrs_col = f"{organ}_mri_mi_clock_acceleration_years"
    age_col = f"{organ}_mri_mi_clock_age_years"
...
def main():
    ...
    print(f"Building {organ} MRI mi clock")
    ...
    print("Final prospective imaging mi dataset:")
    print(f"  N = {df.shape[0]}")
    print(f"  Incident mi events after imaging = {int(df['event'].sum())}")
    ...
    dataset_cols = ["participant_id", "baseline_date", "imaging_date", "mi_date", "admin_censor_date", "end_date", "time_days", "time_years", "event", "age_at_imaging", "sex"]
    ...
    def make_pred_frame(part, split, risk):
        base = ["participant_id", "imaging_date", "mi_date", "admin_censor_date", "end_date", "time_years", "event", "age_at_imaging", "sex"]
