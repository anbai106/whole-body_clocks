#!/usr/bin/env python3
"""
General organ metabolomics mi clock using elastic-net Cox survival modeling.
...
Output naming is controlled by --organ, e.g. --organ heart creates:
  heart_metabolomics_mi_clock_predictions.tsv
  heart_metabolomics_mi_clock_performance.json
  etc.
"""
...
def output_prefix(organ):
    return f"{organ}_metabolomics_mi_clock"
...
def add_clock_age_and_acceleration(pred_df, organ, covariate_cols):
    df = pred_df.copy()
    risk_col = f"{organ}_metabolomics_mi_risk_score"
    z_col = f"{organ}_metabolomics_mi_clock_acceleration_z"
    yrs_col = f"{organ}_metabolomics_mi_clock_acceleration_years"
    age_col = f"{organ}_metabolomics_mi_clock_age_years"
...
def main():
    ...
    print(f"Building {organ} metabolomics mi clock")
