...
for (organ in organ_list) {
  output_dir_final <- paste(output_dir, paste0(organ, "_mri_copd_clock"), 'fastGWA', 'data', sep = "/")
...
### 1 ProtBAG
organ_list = c('Reproductive_female')
for (organ in organ_list) {
  output_dir_final <- paste(output_dir, paste0(organ, "_proteomics_copd_clock"), 'fastGWA', 'data', sep = "/")
  data_src = paste(output_dir_final, 'EPOCH_pheno.txt', sep='/')
  cov_src = paste(output_dir_final, 'EPOCH_cov.txt', sep='/')
  lm_imaging_data(data_src, cov_src, output_dir_final)
}
