# ============================================================
# COPD L'EPOCH model performance visualization
...
#
# Example:
#   brain_mri_copd_clock/
#     brain_mri_copd_clock_predictions.tsv
#     brain_mri_copd_clock_performance.json
#     brain_mri_copd_clock_model_comparison.tsv
#     brain_mri_copd_clock_incremental_value_delta_cindex.tsv
# ============================================================
...
combined_outdir <- file.path(base_dir, "all_copd_lepoch_model_performance")
...
make_manifest <- function() {
  ...
  bind_rows(
    tibble(
      ...
      folder = paste0(stringr::str_to_lower(mri_organs), "_mri_copd_clock"),
      prefix = paste0(stringr::str_to_lower(mri_organs), "_mri_copd_clock")
    ),
    tibble(
      modality = "Proteomics",
      ...
      folder = paste0(proteomics_organs, "_proteomics_copd_clock"),
      prefix = paste0(stringr::str_to_lower(proteomics_organs), "_proteomics_copd_clock")
    ),
    tibble(
      ...
      folder = paste0(metabolomics_organs, "_metabolomics_copd_clock"),
      prefix = paste0(stringr::str_to_lower(metabolomics_organs), "_metabolomics_copd_clock")
    )
  ) %>%
    mutate(
...
message("Expected COPD L'EPOCH clocks: ", nrow(clock_manifest))
...
calc_cindex <- function(df) {
  ...
  # Larger L'EPOCH risk score means higher copd risk, so use -risk_score.
...
# ============================================================
# 4. COPD L'EPOCH column detection
# ============================================================

detect_copd_columns <- function(pred, meta) {
  feature_token <- paste0(meta$organ_key, "_", meta$modality_key)

  expected_risk_score_col <- paste0(feature_token, "_copd_risk_score")
  expected_accel_z_col <- paste0(feature_token, "_copd_clock_acceleration_z")
  expected_accel_year_col <- paste0(feature_token, "_copd_clock_acceleration_years")
  expected_clock_age_col <- paste0(feature_token, "_copd_clock_age_years")
  ...
  risk_score_col <- first_existing_col(pred, c(
    ...
    paste0(feature_token, "_copd_lepoch_risk_score")
  ))
  ...
  if (is.na(risk_score_col)) {
    risk_score_col <- first_regex_col(
      pred,
      patterns = c(
        paste0("^", feature_token, ".*copd.*risk.*score$"),
  ...
  accel_z_col <- first_existing_col(pred, c(
    ...
    paste0(feature_token, "_copd_acceleration_z"),
  ...
  if (is.na(accel_z_col)) {
    accel_z_col <- first_regex_col(
      pred,
      patterns = c(
        paste0("^", feature_token, ".*copd.*acceleration.*z$"),
  ...
  accel_year_col <- first_existing_col(pred, c(
    ...
    paste0(feature_token, "_copd_acceleration_years"),
  ...
  clock_age_col <- first_existing_col(pred, c(
    ...
    paste0(feature_token, "_copd_clock_age_years"),
...
plot_one_clock <- function(meta_row, horizon_years = 5, n_boot = 500, skip_missing = TRUE) {
  ...
  if (!file.exists(meta$clock_dir) || !file.exists(meta$prediction_file) || !file.exists(meta$performance_file)) {
    msg <- paste0(
      "Missing input for ", meta$clock_label, " COPD L'EPOCH:\n",
  ...
  message("\n============================================================")
  message("Processing ", meta$clock_label, " COPD L'EPOCH")
  ...
  detected <- detect_copd_columns(pred, meta)
  ...
  if (is.na(risk_score_col) || !risk_score_col %in% colnames(pred)) {
    stop(
      "Could not identify COPD L'EPOCH risk-score column for ",
      meta$clock_label,
  ...
  message("COPD events total: ", sum(pred$event, na.rm = TRUE))
  ...
  # Preserve original per-clock folder, but use copd-specific figure suffix.
  ...
  if (sum(test_for_km$event, na.rm = TRUE) >= 2) {
    ...
    km_df <- tibble()
    warning(
      "Too few test-set copd events for Kaplan-Meier by quartile for ",
      meta$clock_label,
      "."
    )
  }
  ...
  p_density <- pred %>%
    ...
    labs(
      ...
      x = glue("{meta$clock_label} COPD L'EPOCH risk score"),
      y = "Density"
    ) +
    theme_clock()
  ...
  if (nrow(km_df) > 0) {
    p_km <- km_df %>%
      ...
      labs(
        title = "D. Test-set copd separation",
        subtitle = "Kaplan-Meier cumulative copd incidence by training-defined risk quartile",
        x = glue("Years after {time_origin_label}"),
        y = "Cumulative copd incidence"
      ) +
      theme_clock()
  } else {
    p_km <- ggplot() +
      annotate(
        ...
        x = 0,
        y = 0,
        label = "Kaplan-Meier panel unavailable: too few test-set copd events.",
        size = 4.5,
        fontface = "bold"
      ) +
      theme_void() +
      labs(title = "D. Test-set copd separation")
  }
  ...
  p_quartile <- quartile_risk_tbl %>%
    ...
    scale_y_continuous(labels = percent_format(accuracy = 0.1)) +
    labs(
      title = glue("E. Observed {local_horizon_years}-year copd risk by risk quartile"),
      subtitle = "Monotonic validation/test patterns support generalization",
      x = NULL,
      y = glue("Observed {local_horizon_years}-year copd risk")
    ) +
    theme_clock() +
  ...
  if (nrow(cal_tbl) > 0) {
    ...
    p_cal <- cal_tbl %>%
      ...
      labs(
        ...
        subtitle = "Observed Kaplan-Meier copd risk versus predicted Cox risk by decile",
        x = glue("Mean predicted {local_horizon_years}-year copd risk"),
        y = glue("Observed {local_horizon_years}-year copd risk")
      ) +
      theme_clock()
  } else {
  ...
  subtitle_text <- glue(
    "N = {nrow(pred)}, copd events = {sum(pred$event)}, ",
  ...
  final_fig <-
    ...
    (p_quartile | p_cal) +
    plot_annotation(
      title = glue("{meta$clock_label} COPD L'EPOCH performance"),
      subtitle = subtitle_text,
      caption = paste0(
        "Risk quartiles are defined from the training set and applied to validation/test. ",
        "Higher L'EPOCH risk score indicates greater copd-event proximity."
      ),
      theme = theme(
...
# ============================================================
# 6. Loop over all COPD L'EPOCH clocks
# ============================================================
...
if (length(results) == 0) {
  stop("No COPD L'EPOCH clocks were successfully processed.")
}
...
out_combined_summary <- file.path(
  combined_outdir,
  "all_copd_lepoch_summary.tsv"
)

out_combined_overfit <- file.path(
  combined_outdir,
  "all_copd_lepoch_overfitting_summary.tsv"
)

out_combined_quartile <- file.path(
  combined_outdir,
  "all_copd_lepoch_risk_quartile_summary.tsv"
)

out_combined_calibration <- file.path(
  combined_outdir,
  "all_copd_lepoch_calibration_summary.tsv"
)

out_combined_cindex <- file.path(
  combined_outdir,
  "all_copd_lepoch_cindex_summary.tsv"
)

out_combined_incremental <- file.path(
  combined_outdir,
  "all_copd_lepoch_incremental_value_summary.tsv"
)

message("\n============================================================")
message("Writing combined COPD L'EPOCH outputs")
message("============================================================")
...
base_quality <- combined_summary_plot_tbl %>%
  ...
  labs(
    title = "D. Practical COPD L'EPOCH model-quality map",
...
combined_fig <-
  ...
  plot_annotation(
    title = "Performance summary of MRI, proteomics, and metabolomics COPD L'EPOCH models",
    subtitle = glue(
      "Processed {nrow(combined_summary)} of {nrow(clock_manifest)} expected COPD L'EPOCH models. ",
...
out_combined_pdf <- file.path(
  combined_outdir,
  "all_copd_lepoch_combined_model_performance.pdf"
)

out_combined_png <- file.path(
  combined_outdir,
  "all_copd_lepoch_combined_model_performance.png"
)
...
message("  - Per-clock PDF/PNG figures and summary TSVs are saved in each COPD L'EPOCH folder.")
